...
const GSTR1B2CSAmendmentSummary = () => {
            ...
            try {
                ...
                if (res.data.success && res.data.data) {
                    ...
                    invoices.forEach(inv => {
                        if (inv.pos) posSet.add(inv.pos);
                    });
                    setAvailablePOS(Array.from(posSet).sort());
                }
            } catch (error) {
                console.error("Failed to fetch B2CS records", error);
            }
        };
        fetchInvoices();
    }, []);
    ...
    const handleAmend = async () => {
        ...
        try {
            ...
            // The prompt asks to search by Year + Month + Original POS. 
            // In a real GST portal, it searches by POS (and sometimes rate) for B2CS. We'll search by POS.
            const foundRecord = savedInvoices.find(inv => inv.pos === originalPOS);

            if (foundRecord) {
                toast.success('Record found! Navigating to amend details...');
                navigate('/returns/auth/gstr1/b2cs/amendment/details', { state: { invoice: foundRecord, year: financialYear, month, isDifferential } });
            } else {
                toast.error('No matching amended B2C(Others) record found.');
            }
        } catch (error) {
        ...
        <div className="dashboard-container" style={{ backgroundColor: '#f1f3f6', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="dashboard-breadcrumb-bar">
                <div className="breadcrumb-left">
                    ...
                    <span style={{ color: '#9ca3af', margin: '0 5px' }}>&gt;</span>
                    <span style={{ color: '#4b5563' }}>B2CSA</span>
                </div>
            ...
            <div className="gstr1-main-content" style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto', width: '100%', flex: '1' }}>
                ...
                <div className="b2b-header-banner" style={{ backgroundColor: '#14b8a6', padding: '10px 15px', color: '#fff', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: '15px' }}>
                    <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '500' }}>10 - Amended B2C(Others)</h2>
                ...
                <div style={{ backgroundColor: '#e0f2fe', padding: '15px', marginTop: '15px', color: '#0369a1', fontSize: '13px', border: '1px solid #bae6fd', fontStyle: 'italic' }}>
                    <p style={{ margin: '0 0 5px 0' }}>To amend the POS of an already filed record (as reported in table 7 of earlier return period), you are required to use the following steps:</p>
...
export default GSTR1B2CSAmendmentSummary;
