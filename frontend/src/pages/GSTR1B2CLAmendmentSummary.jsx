...
import './GSTR1B2BDashboard.css'; // Reusing for common header banner styles if needed

const GSTR1B2CLAmendmentSummary = () => {
    ...
    const handleAmend = async () => {
        if (!invoiceNo.trim()) {
            setError('Please enter Invoice No');
            return;
        }
        setError('');
        
        // Search the database for the entered invoice number in saved B2CL records
        setLoading(true);
        try {
            ...
            if (res.data.success && res.data.data) {
                const invoices = Array.isArray(res.data.data.invoices) ? res.data.data.invoices : (Array.isArray(res.data.data) ? res.data.data : []);
                foundInvoice = invoices.find(inv => inv.invoiceNo === invoiceNo.trim());
            }

            if (foundInvoice) {
                // Navigate to the B2CL Amendment Details page and load the existing invoice data for editing
                // Pass state or query param. For now passing in state.
                toast.success('Invoice found! Navigating to amend details...');
                navigate('/returns/auth/gstr1/b2cl/amendment/details', { state: { invoice: foundInvoice } });
            } else {
        ...
        <div className="dashboard-container" style={{ backgroundColor: '#f1f3f6', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="dashboard-breadcrumb-bar">
                <div className="breadcrumb-left">
                    ...
                    <span style={{ color: '#9ca3af', margin: '0 5px' }}>&gt;</span>
                    <span style={{ color: '#4b5563' }}>B2CLA</span>
                </div>
            ...
            <div className="gstr1-main-content" style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto', width: '100%', flex: '1' }}>
                ...
                <div className="b2b-header-banner" style={{ backgroundColor: '#14b8a6', padding: '10px 15px', color: '#fff', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: '15px' }}>
                    <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '500' }}>9A - Amended B2C (Large) Invoices</h2>
                    <div className="gstr1-header-actions" style={{ display: 'flex', gap: '10px' }}>
                        ...
                        <button className="gstr1-btn-icon" style={{ backgroundColor: '#14b8a6', border: '1px solid #0f766e', color: 'white', padding: '4px 8px', cursor: 'pointer' }} onClick={handleRefresh}>↻</button>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '40px', gap: '10px' }}>
                    ...
                    <button 
                        onClick={handleAmend}
                        style={{ padding: '8px 20px', backgroundColor: '#2563eb', border: 'none', color: '#fff', cursor: 'pointer', fontWeight: '500' }}
                    >
                        AMEND RECORD
                    </button>
                </div>
...
export default GSTR1B2CLAmendmentSummary;
